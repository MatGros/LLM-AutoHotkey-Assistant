...
// ========================================
// Auto-resize: notify AHK of desired window size
// ========================================
...
function requestResize() {
  ...
  if (!contentEl || !pageContent) return;

  // Get the content's scroll height plus buttons/padding
  var contentHeight = contentEl.scrollHeight;
  var buttonsHeight = 50; // approximate button bar + margins
  var padding = 60; // page-content padding + margins + border

  var desiredHeight = contentHeight + buttonsHeight + padding;

  // Clamp between min and max
  var minH = 200;
  var maxH = Math.floor(screen.height * 0.75);
  desiredHeight = Math.max(minH, Math.min(desiredHeight, maxH));

  // Desired width: based on content, clamped
  var desiredWidth = Math.max(500, Math.min(contentEl.scrollWidth + padding, Math.floor(screen.width * 0.5)));

  // Send to AHK
  if (typeof ahkHandler !== 'undefined') {
    ahkHandler.Func(JSON.stringify({ action: 'resize', width: desiredWidth, height: desiredHeight }));
  }
}