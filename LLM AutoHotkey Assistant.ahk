...
mainScriptHotkeyActions(action) {
    ...
    switch action {
        case "showPromptMenu":
            ...
            ; Normal prompts — DEBUG: n'afficher que la catégorie "Général" pour faciliter le debugging
            for index, prompt in managePromptState("prompts", "get") {

                ; Only include prompts that are explicitly tagged with 'Général' (case-insensitive)
                includePrompt := false
                if (prompt.HasProp("tags") && prompt.tags && prompt.tags.Length > 0) {
                    for tag in prompt.tags {
                        if InStr(StrLower(tag), "général") {
                            includePrompt := true
                            break
                        }
                    }
                ...
                ; Skip any prompt that is not part of the 'Général' category
                if (!includePrompt)
                    continue

                ; Ensure the 'Général' submenu exists and add the prompt there
                genKey := "&général"
                if !tagsMap.Has(genKey) {
                    tagsMap[genKey] := { menu: Menu(), displayName: "&Général" }
                    promptMenu.Add("&Général", tagsMap[genKey].menu)
                }

                tagsMap[genKey].menu.Add(prompt.menuText, promptMenuHandler.Bind(index))
            }
...
; Verify curl is available on PATH — helpful to fail early with a clear message
checkCurlAvailability()

checkCurlAvailability() {
    tmp := A_Temp "\where_curl_check.txt"
    try {
        ; Build command safely and run it
        RunWait(Format('cmd /c where curl.exe > "{}" 2>nul', tmp))
        if (!FileExist(tmp) || FileOpen(tmp, "r").Read() = "") {
            MsgBox("Warning: 'curl.exe' not found in PATH. The script requires curl for API calls. Please install curl or add it to PATH.", "Missing dependency", 48)
        }
    } catch {
        ; ignore errors
    }
    ...
    try {
        ...
        for _, p in promptsList {
            if (p.HasProp("APIModels") && p.APIModels) {
                ; Remove whitespace AND parentheses so model names like "(gemma3:4b)" become "gemma3:4b"
...
processInitialRequest(promptName, menuText, systemPrompt, APIModels, copyAsMarkdown, isAutoPaste, skipConfirmation,
    ...
    ; Removes newlines, spaces and parentheses, then splits by comma
    ...
    ; Kill ALL Response Window processes before opening a new one
    ; Use WMI to find all AutoHotkey processes running Response Window.ahk
    for proc in ComObjGet("winmgmts:").ExecQuery("SELECT ProcessId, CommandLine FROM Win32_Process WHERE Name LIKE 'AutoHotkey%'") {
        if (InStr(proc.CommandLine, "Response Window.ahk")) {
            ProcessClose(proc.ProcessId)
        }
    }
    Sleep 150  ; Give processes time to fully close
    
    ; Clear the activeModels map
    getActiveModels().Clear()

    for i, fullAPIModelName in APIModels {

        ; If provider prefix (e.g. "ollama/") is present, split it; otherwise assume local 'ollama' provider
        if (InStr(fullAPIModelName, "/")) {
        ...
        ; Remove non-ASCII characters and replace spaces with underscore to avoid encoding issues in external commands
        safePromptName := RegExReplace(promptName, "[^\x00-\x7F]", "")
        safePromptName := StrReplace(safePromptName, " ", "_")

        chatHistoryJSONRequestFile := A_Temp "\" RegExReplace("chatHistoryJSONRequest_" safePromptName "_" singleAPIModelName "_" uniqueID ".json",
            "[\/\\:*?`"<>|]", "")
        cURLCommandFile := A_Temp "\" RegExReplace("cURLCommand_" safePromptName "_" singleAPIModelName "_" uniqueID ".txt",
            "[\/\\:*?`"<>|]", "")
        cURLOutputFile := A_Temp "\" RegExReplace("cURLOutput_" safePromptName "_" singleAPIModelName "_" uniqueID ".json",
            "[\/\\:*?`"<>|]", "")
        ...
        Run("lib\Response Window.ahk " "`"" dataObjToJSONStrFile, , , &responseWindowPID)
        getActiveModels()[uniqueID].PID := responseWindowPID
    }
}
